    ...
    async create() {
        ...
        if (data && data.success) {
            ...
            if (data.active_pet) {
                ...
                // Thanh Độ Đói
                let hungerColor = p.hunger < 20 ? '#ff0000' : '#00ff00';
                this.add.text(width / 2, 250, `❤️ Độ no: ${p.hunger}/100`, { 
                    fontSize: '18px', fill: hungerColor, stroke: '#000', strokeThickness: 3 
                }).setOrigin(0.5);
        ...
        // Đã căn chỉnh tọa độ để không bị đè
        let huntBtn = this.add.text(width / 2, height - 290, "[ ⚔️ ĐI SĂN ]", { 
            fontSize: '28px', fontStyle: 'bold', fill: '#ffffff', backgroundColor: '#8b0000', padding: 10, stroke: '#000', strokeThickness: 4
        }).setOrigin(0.5).setInteractive();
        huntBtn.on('pointerdown', () => this.scene.start('HuntScene', { userId: userId, userData: data }));

        let petBtn = this.add.text(width / 2, height - 230, "[ 🦁 CHUỒNG PET ]", { 
            fontSize: '24px', fontStyle: 'bold', fill: '#fff', backgroundColor: '#00008b', padding: 10, stroke: '#000', strokeThickness: 4
        }).setOrigin(0.5).setInteractive();
        petBtn.on('pointerdown', () => this.scene.start('PetScene', { userId: userId }));

        let invBtn = this.add.text(width / 2, height - 170, "[ 🎒 TÚI ĐỒ ]", { 
            fontSize: '24px', fontStyle: 'bold', fill: '#fff', backgroundColor: '#555500', padding: 10, stroke: '#000', strokeThickness: 4
        }).setOrigin(0.5).setInteractive();
        invBtn.on('pointerdown', () => this.scene.start('InventoryScene', { userId: userId }));

        let craftBtn = this.add.text(width / 2, height - 110, "[ 🔥 LÒ RÈN ]", { 
            fontSize: '24px', fontStyle: 'bold', fill: '#fff', backgroundColor: '#aa3300', padding: 10, stroke: '#000', strokeThickness: 4
        }).setOrigin(0.5).setInteractive();
        craftBtn.on('pointerdown', () => this.scene.start('CraftScene', { userId: userId }));

        let marketBtn = this.add.text(width / 2, height - 50, "[ ⚖️ CHỢ ĐEN ]", { 
            fontSize: '24px', fontStyle: 'bold', fill: '#fff', backgroundColor: '#004444', padding: 10, stroke: '#000', strokeThickness: 4
        }).setOrigin(0.5).setInteractive();
        ...
        if (ADMIN_IDS.includes(userId)) {
            let adminBtn = this.add.text(width - 20, 20, "🛠", { 
                fontSize: '30px', backgroundColor: '#aa0000', padding: 10 
